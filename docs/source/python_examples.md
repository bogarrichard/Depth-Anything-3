...
{class}`~depth_anything_3.api.DepthAnything3`. See {doc}`reference/config_system` for
the full `load_config`/`create_object` reference. To try a new
architecture, write a config file:
