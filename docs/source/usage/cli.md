...
```{seealso}
For the full, auto-generated list of every subcommand's options (kept in
sync with the code, since it's generated straight from the CLI's own option
declarations), see {doc}`../reference/cli`. This page is a task-oriented
walkthrough. `da3 <command> --help` always works too.
```
